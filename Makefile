...
run-docling-cpu: ## Run the docling-serve container with CPU support and assign a container name
	...
	$(CMD_PREFIX) docker run -it --name docling-serve-cpu -p 5001:5001 -e DOCLING_SERVE_ENABLE_UI=true ghcr.io/docling-project/docling-serve-cpu:main
...
run-docling-gpu: ## Run the docling-serve container with GPU support and assign a container name
	...
	$(CMD_PREFIX) docker run -it --name docling-serve-gpu -p 5001:5001 -e DOCLING_SERVE_ENABLE_UI=true
